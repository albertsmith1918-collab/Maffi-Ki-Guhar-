...
function classMatchesRegex(node, regex, checkAncestors) {
  ...
  if (checkAncestors) {
    return (
      distanceToMatch(node, (node2) =>
        elementClassMatchesRegex(node2, regex)
      ) >= 0
    );
...
function serializeElementNode(n, options) {
  ...
  if (
    tagName === "style" &&
    n.sheet && // TODO: Currently we only try to get dynamic stylesheet when it is an empty style element
...
function serializeNodeWithId(n, options) {
  ...
  if (
    (serializedNode2.type === NodeType.Document ||
      serializedNode2.type === NodeType.Element) &&
    recordChild
  ) {
    if (
